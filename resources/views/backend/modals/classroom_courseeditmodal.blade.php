              ...
              <select class="form-control" name="course_category_id" required>
               ...
               @foreach ($course_categories as $course_category)
                                <option value="{{$course_category->id }}"
                                    {{ $course_category->id == $row->course_category_id ? 'selected' : '' }}>
                                    {{ ucwords($course_category->mcategory_title) }}</option>
                            @endforeach
              ...
              <select class="form-control" name="main_category_id" required>
                ...
                @foreach ($main_categories as $main_category)
                                <option value="{{ $main_category->id }}"
                                    {{ $main_category->id == $row->main_category_id ? 'selected' : '' }}>
                                    {{ ucwords( $main_category->mcategory_title ) }}</option>
                            @endforeach
